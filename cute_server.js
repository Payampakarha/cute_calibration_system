...
var avrSN = [
    'ffffffff3850313339302020ff0d0b' // calibration AVR0
];
...
var calibrate = [
    ...
    [ 22633, 0, 24935, 0.3302, 28032, 0.8128, 30308, 1.143, 33553, 1.6256, 35613, 1.9558 ], // mm - damper A top position (0 = at floor)
    [ 21123, 0, 23328, 0.3302, 26498, 0.8128, 28631, 1.143, 31628, 1.6256, 33889, 1.9558 ], // mm - damper B top position (0 = at floor)
    [ 22753, 0, 24888, 0.3302, 28222, 0.8128, 30334, 1.143, 33712, 1.6256, 35631, 1.9558 ], // mm - damper C top position (0 = at floor)

    // channels 3-5: 0-5 V; hall-effect linear sensors
    [14816, 22.42, 16917, 21.18, 18673, 20.2, 22228, 18.34, 25277, 16.81, 28388, 15.38, 31021, 14.16, 33293, 13.21, 36955, 11.59, 40521, 9.98, 43163, 8.61, 45827, 7.19, 48151, 5.92, 49952, 4.94, 51599, 4.05, 53725, 2.94, 55985, 1.68, 57698, 0.72, 58762, 0], // mm - lab jack A top position (0 = 35 kg load when at floor)
    [18797, 19.25, 22138, 17.62, 25108, 16.15, 28049, 14.58, 31384, 12.87, 34442, 11.23, 37690, 9.6,  40839, 8.04, 44319, 6.42, 48137, 4.84, 51975, 3.19, 55711, 1.57, 59346, 0], // mm - lab jack B top position (0 = 35 kg load when at floor)
    [ 17570, 21.71, 19849, 20.61, 21832, 19.67, 24201, 18.56, 26357, 17.54, 28478, 16.5, 31059, 15.11, 33247, 13.93, 35191, 12.84, 40873, 9.5, 42771, 8.47, 45278, 7.04, 47408, 5.9, 49926, 4.6, 51496, 3.77, 54085, 2.42, 56653, 0.99, 58429, 0], // mm - lab jack C top position (0 = 35 kg load when at floor)

    // channel 6: 0-5 V; atmospheric pressure sensor (hPa)
    [ 643.564, 16.51295, 7170.949, 188.65442, 13648.527, 358.91363, 20249.313, 532.55816, 26817.331, 705.265, 33386.66, 878.24763, 39841.956, 1047.96215, 46476.820, 1222.79258, 52958.331, 1566.13094, 59535.524, 1566.13094, 65916.109, 1733.75635 ],  // hPa - nominal transducer calibration: 5V=25psi

    // channel 7: 0-5 V; liquid level sensor (converted to m of H2O)
    [ 52.4288, 0, 32833.536, 3.52472, 65601.536, 7.0494 ]
...
//calibration system vars
var current_pos = "?"; //current position of the source
var current_speed = "?"; //current source speed
var security_status = "?"; //current status 0 or 1
var user ="cute"; //current user 
var update_logfile_frequency = 1000; //in ms, how fast you want to write on log file
//-----------------------------------------------------------------------------
// Main script
...
//setInterval(LogToFile,update_logfile_frequency); //Payam, this is moved to a php code.
//-----------------------------------------------------------------------------
// poll the hardware
...
function parse_message(args)
{
    ...
    var temp_msg = ""; 
     //parsing position and status queries 
    if (locate_pos != -1 ) {
        temp_msg = msg.substr(locate_pos);
        current_pos = temp_msg.substr(4);
    }
    else if (locate_val !=-1 ){
        temp_msg = msg.substr(locate_val);
        temp_message = temp_msg.substr(4);
        if ( temp_message == "0" ) {
...
function LogToFile() 
{
    ...
    tolog = d.getFullYear() + '-' + Pad2(d.getMonth()+1) + '-' + Pad2(d.getDate()) + '\t ' + Pad2(d.getHours()) + ':' + Pad2(d.getMinutes()) + ':' + Pad2(d.getSeconds());
    //adding values to the log
    tolog += "\t" + user;
    tolog += "\t" + current_pos;
    tolog += "\t " + security_status;
    var filename = '' + d.getFullYear()+'-' +(d.getMonth()+1)+'-'+d.getDate()+'.txt';
    if (!(fs.existsSync(filename))) {
        fs.appendFile(filename, "[Date] \t [Time] \t [User] \t [current source position a.u] \t [security status] \n");
        fs.appendFile(filename,'**********************************************************************************\n');
    }
    else {
        fs.appendFile(filename, tolog + '\n',
            function(error) {
...
/* This function decides what is going to be logged, parsing them from messages from server */ 
function parse_log_from_server_msg() 
{
    return tolog;
}
...
function Log(args)
{
    var msg = parse_message.apply(this, arguments);
    // log to console
    console.log(msg);
